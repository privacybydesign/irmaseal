...
impl<R: AsyncRead + Unpin> OpenerSealed<R> {
    ...
    pub async fn new(mut r: R) -> Result<(Metadata, Self), Error> {
        ...
        let mut metadata_bytes = [0u8; MAX_METADATA_SIZE];
        r.read_exact(&mut metadata_bytes[..meta_len])
            .map_err(|err| Error::ReadError(err))
            .await?;
        metadata_buf.write(&metadata_bytes[..meta_len]);

        let metadata = postcard::from_bytes(&metadata_bytes).or(Err(Error::FormatViolation))?;
    ...
    pub async fn unseal<W: AsyncWrite + Unpin>(
        ...
        let mut buf = [0u8; BLOCKSIZE + MACSIZE]; // TODO: size?
